...
import { Copy, Trash2, ArrowRight, ShieldCheck, Sparkles } from 'lucide-react';
...
export function ResumeCard({
  ...
  const hasDigiLocker = resume.education?.some((edu) => edu.digilocker_verified);
      ...
      >
        <div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            {hasDigiLocker && (
              <span title="DigiLocker Verified" className="text-emerald-600 shrink-0">
  ...
  return (
    ...
    >
      <div>
        ...
        <Link href={`/dashboard/resumes/${resume.id}`} className="block">
          <div className="flex items-center gap-1.5">
            <h3 className="font-bold text-gray-900 text-base group-hover:text-indigo-600 transition-colors truncate flex-1">
              {resume.title}
            </h3>
            {hasDigiLocker && (
              <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200 px-1.5 py-0.5 rounded text-[10px] font-bold shrink-0">
